...
function _showZoomHint(){
  let h=document.getElementById('tl-zoom-hint');
  if(!h){
    h=document.createElement('div');h.id='tl-zoom-hint';h.className='tl-zoom-hint';
    h.textContent='Ctrl + scroll para zoom';
    document.getElementById('tl-panel')?.appendChild(h);
  }
  h.classList.add('visible');
  clearTimeout(h._t);
  h._t=setTimeout(()=>h.classList.remove('visible'),1800);
}
